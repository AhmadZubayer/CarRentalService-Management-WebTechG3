...
<div class="profile-max-width">
    <div class="container-card">
        <div class="card-heading">Profile Management</div>
    ...
    <div class="container-card" style="margin-top: 30px;">
        <div class="card-heading">Rental History</div>
        <div class="tbl-wrap">
            <table>
                <thead>
                    <tr>
                ...
                <tbody id="orderHistoryBody">
                    <tr>
                        <td colspan="5" style="text-align:center; padding: 30px; color: #9ca3af;">Loading history...</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
function loadOrderHistory() {
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            if (!data || data.length === 0) {
                html = '<tr><td colspan="5" style="text-align:center; padding: 30px; color: #9ca3af;">No orders found.</td></tr>';
            } else {
                data.forEach(function(order) {
                    html += '<tr>';
                    html += '<td>#' + order.id + '</td>';
                    html += '<td><strong>' + order.car_name + '</strong><br><span style="font-size:11px; color:#6b7280;">' + order.car_type + ' - ' + order.car_model + '</span></td>';
                    html += '<td style="font-size:13px;">' + formatDate(order.start_date) + ' - ' + formatDate(order.end_date) + '</td>';
                    html += '<td style="font-weight:600;">Tk ' + parseFloat(order.total_cost).toLocaleString() + '</td>';
                    html += '<td><span class="badge badge-' + order.status + '">' + order.status + '</span></td>';
                    html += '</tr>';
                });
            }
    ...
    xhr.send('module=order&action=getUserOrders&csrf_token=' + encodeURIComponent(window.csrfToken));
}
